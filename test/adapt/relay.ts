...
describe('Adapt/Relay', () => {
  ...
  it('Should receive ERC721', async () => {
    const { relayAdapt, testERC721, secondaryAccount } = await loadFixture(deploy);
    // mint tokens
    ...
    expect(
      await relayAdapt.connect(secondaryAccount).receivedERC721(testERC721.address, 0),
    ).to.equal(10);
    expect(
      await relayAdapt.connect(secondaryAccount).receivedERC721(testERC721.address, 1),
    ).to.equal(11);
    ...
    await expect(
      relayAdapt
        .connect(secondaryAccount)
        .onERC721Received(secondaryAccount.address, secondaryAccount.address, 10n, []),
    ).to.be.revertedWith('msg.sender is not an ERC721 contract!');
  });
