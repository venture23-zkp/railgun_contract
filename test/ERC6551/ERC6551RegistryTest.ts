...
describe('ERC6551 Registry', function () {
  ...
  describe('Deployment', function () {
    it('Should return valid name and symbol', async function () {
      ...
      expect(await registry.name()).to.equal('Registry NFT');
      expect(await registry.symbol()).to.equal('RNFT');
    });
  });
  ...
  describe('Create Account,Mint NFT', function () {
    it('Should create account and mint nft', async function () {
      ...
      const create = await registry.createAccount(
        account.address,
        chainId,
        tokenAddress,
        0,
      ...
      const create2 = await registry.createAccount(
        account.address,
        chainId,
        tokenAddress,
        0,
  ...
  describe('Failed Create Account,Mint NFT', function () {
    it('Should revert while creating account and mint nft', async function () {
      const { registry, account } = await loadFixture(deployContracts);
      ...
      await expect(
        registry.createAccount(account.address, chainId, zeroAddress, 0, initData),
      ).to.be.revertedWithCustomError(registry, 'InitializationFailed');
      ...
      await expect(
        registry.createAccount(account.address, chainId, tokenAddress, tokenId, initData),
      ).to.be.revertedWith('createAccount caller is not an owner!');
    });
  });
});
